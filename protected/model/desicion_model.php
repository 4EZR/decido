...
class Decision_Model {
    ...
    public function get_decision_matrix_data($id){
        ...
        $stmt = $this->db->prepare("SELECT c.Criteria_Type,c.Criteria_Title, c.Criteria_Importance, lt.TermLevel_1, lt.TermLevel_2, lt.TermLevel_3, lt.TermLevel_4, lt.TermLevel_5, aw.Weight,c.Criteria_Importance,c.Criteria_ID, aw.Alternative_ID,a.Alternative_Title FROM criterias c JOIN linguistic_terms lt ON c.Linguistic_Term = lt.Term_ID JOIN alternative_weight aw ON c.criteria_ID = aw.Criteria_ID inner join alternatives a on aw.Alternative_ID = a.Alternative_ID WHERE c.Decision_ID = ? ORDER BY aw.Alternative_ID ASC, c.Criteria_Importance ASC");
        $stmt->execute([$id]);
